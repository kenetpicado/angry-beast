...
function destroy(id) {
    confirm('¿Estas seguro de eliminar este usuario?') && form.delete(route('dashboard.users.destroy', id), {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            deleted()
            ...
            <form class="bg-white rounded-xl px-7 py-5 w-full flex flex-col" style="max-width: 50rem; min-height: 20rem"
                ...
                <div class="flex-1">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <InputForm v-model="form.password" label="Contraseña" name="password" required
                                   type="password">
                            <IconLock class="text-stroke" size="25" stroke="2"/>
                        </InputForm>

                        <InputForm v-model="form.password_confirmation" label="Confirmar contraseña"
                                   name="password_confirmation"
                                   required type="password">
                            <IconLock class="text-stroke" size="25" stroke="2"/>
                        </InputForm>

                        <div v-if="passwordStatus && form.password" class="mb-2 text-red-400 text-sm col-span-2">
                            {{ passwordStatus }}
                        </div>
                    </div>
                </div>
